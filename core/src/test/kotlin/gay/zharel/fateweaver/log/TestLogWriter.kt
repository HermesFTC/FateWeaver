...
class TestLogWriter {
    ...
    fun testLogWriterIntegration(@TempDir tempDir: Path) {
        ...
        val writer = FateLogWriter(FileOutputStream(logFile))
    ...
    fun testLogWriterAutoClose() {
        ...
        FateLogWriter(outputStream).use { writer ->
            writer.write(channel, 42)
        }
    ...
    fun testLogWriterFactoryMethods() {
        val tempFile = createTempFile("test", ".log").toFile()
        tempFile.deleteOnExit()

        // Test factory method with File
        assertDoesNotThrow {
            FateLogWriter.create(tempFile).use { writer ->
                val channel = writer.createChannel("test", IntSchema)
                channel.put(42)
            }
        }
        ...
        assertDoesNotThrow {
            FateLogWriter.create(tempFile.absolutePath).use { writer ->
                val channel = writer.createChannel("test", StringSchema)
                channel.put("hello")
            }
        }
    ...
    fun testChannelOwnership() {
        ...
        val writer1 = FateLogWriter(outputStream1)
        val writer2 = FateLogWriter(outputStream2)
